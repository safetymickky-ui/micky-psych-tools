...
- **code-explainer** — the other interactive-HTML sibling, split on *what is on screen*: a
  concrete listing the user hands over, explained line by line, is its job. This skill builds a
  running model of a concept and never reproduces someone's source as the artifact. "Explain
  this file" → there; "show me how the algorithm behaves" → here.
