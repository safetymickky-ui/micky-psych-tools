---
name: intent-lock
description: Interrogates a request until it has exactly one reading, then executes it as written. Use when the user says "interview me", "ask me until you understand", "make sure you don't misunderstand", "lock the goal", "craft my prompt", "what do I actually want", "ถามจนกว่าจะเข้าใจ", "ล็อคเป้าหมาย" — and unprompted, offer it whenever a wrong reading of an expensive, reusable, or ambiguous request would waste substantial work. The user may background the thread and return only to finished output, so the interview is the ONLY correction window. Rounds are uncapped: interrogate until saturation or the user says LOCK / stop asking. Never propose a better version of the request, never emit a portable prompt artifact, never publish the convergence gates, never ask permission to proceed — after GOAL UNIFIED, work. Not for quick throwaway or already-precise requests, mid-task execution forks (decision-interview), critiquing or improving an existing plan (plan-critique), or a past misread after delivery (misread-capture).
---
...
- The degenerate-loop guard fired and the interview continued anyway.
- A prompt, spec, or instruction was emitted as an artifact for the user to carry elsewhere.

Full list: `references/failure-conditions.md`.
