...
5. **category** — offer the categories already in `marketplace.json` (currently
   `research`, `productivity`) plus free-text for a new one.
...
Do not commit and do not run `bump.py` — a new plugin starts at `0.1.0`.
