...
`FIRECRAWL_API_KEY` lives in the environment (`.env`, shell profile, or runtime
config) — never in this repo, never in a vault artifact, never in a committed
file. If a key ever appears in chat or a document, treat it as exposed and
rotate it in the Firecrawl dashboard.
