...
description: >-
  ...
  /infographic. Deep-integrates with pubmed-research-note (and comprehensive-review /
  vault-keeper when those plugins are installed): it NEVER invents clinical facts — it renders
  a review or decision report from this session or the vault, and if none exists runs the
  research plugin FIRST to generate sourced content, then lays it out as color-coded columns,
  stat tiles, and a mandatory contraindications / "medications to avoid" safety banner; filed
  to the vault as an asset via vault-keeper when installed. NOT for: producing the underlying
  evidence (the research plugins own that), or non-clinical data charts.
---
...
2. **An existing vault artifact** — query the **vault-keeper** skill for a matching review or
   ...
   of truth. Do not re-research it. *(Only when the vault-keeper plugin is installed;
   standalone, skip to 3 — or render a report file the user points at.)*
3. **No sourced content exists → generate it first, do not fabricate.** Pick the engine by what
   ...
   - a **whole-topic / whole-disorder reference** (the usual case — phases, columns, the full
     picture) → run **comprehensive-review** first, then render its md. *(If that plugin is
     not installed — this kit ships without it — run **pubmed-research-note** here too, scoped
     to the whole reference the user wants.)*
   ...
   Those plugins own their own decision-lock gate, searches, and citation discipline — let
   them. This skill never re-implements the interview and never issues a clinical claim of its
   own.
...
3. **File it via vault-keeper.** An HTML infographic is a rendered **asset**: hand it to the
   **vault-keeper** skill to place in `vault/assets/` and to create/extend a short companion
   ...
   into `vault/` from this skill. Skip only on an explicit "don't vault this" — **or when the
   vault-keeper plugin is not installed** (standalone install of this kit): then steps 1–2 are
   the whole pipeline; say in the Close that no vault filing was attempted. Never improvise a
   vault of your own.
...
- **pubmed-research-note** — generates the sourced decision/protocol content when the
  infographic is about one call, and (standalone) the whole-topic content too. It also owns any
  live verdict — the infographic shows the verdict, it never issues one.
- **comprehensive-review** *(when installed)* — generates the whole-topic sourced content when
  none exists. Never duplicate its search here.
- **vault-keeper** *(when installed)* — every vault write (asset + companion note + MOC
  wiring), per Step 3. Without it, no vault step exists.
- **intent-lock** *(when installed)* — not re-run here; it gates the upstream content plugins
  when they generate.
...
- Content was rendered with no traceable source when pubmed-research-note (or
  comprehensive-review, when installed) should have generated it first.
- A vault path was resolved or a file written into `vault/` by this skill instead of
  vault-keeper, or a save/path was claimed that did not happen — including inventing a vault
  when vault-keeper is not installed.
