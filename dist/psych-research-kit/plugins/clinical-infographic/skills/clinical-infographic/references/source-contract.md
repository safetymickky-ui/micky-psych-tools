...
| | Owns | Never does |
|---|---|---|
| pubmed-research-note | the sourced report — verdict + working, its own citations | visual layout |
| comprehensive-review *(when installed)* | whole-topic sourced content, its own citations | visual layout |
...
2. **An existing vault artifact** *(when the vault-keeper plugin is installed)*. Ask
   **vault-keeper** to `query` for a review or decision report on the topic (title / MOC / tag
   match). A fitting one is the single source of truth — do not re-research a topic the vault
   already answers. Standalone, this step does not exist — but a report file the user points
   at fills the same role.
3. **Nothing sourced exists → generate first.** Choose the engine by what the infographic is:
   - **Whole-topic / disorder reference** (multiple phases or themes, the full picture) →
     **comprehensive-review**. Its md chapter becomes the render skeleton. *(Not installed in
     this kit → use **pubmed-research-note**, scoped to the whole reference.)*
...
- **To pubmed-research-note / comprehensive-review (generate):** the topic or decision, plus a
  note that the output will be rendered as a one-page clinical reference (so emphasis and
  brevity are welcome). Let their own decision-lock gate run — do not pre-empt it.
- **To vault-keeper (file, when installed):** the finished `.html` as an **asset**, a title
  `<Topic> — Infographic`, a short companion-note body (what it summarises, link to the source
  artifact), and the **source report's MOC topic** so the infographic hangs off the same map as
  its source. Without vault-keeper there is no vault step — the HTML and PNG in the working
  directory are the deliverables.
