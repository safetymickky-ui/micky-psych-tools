...
The skill owns the whole procedure — acquire **SOURCED** content (reuse an existing report
from this session or the vault, or run pubmed-research-note — or comprehensive-review, when
installed — FIRST to generate it; never fabricate clinical facts) → render the self-contained,
print-ready HTML infographic with color-coded columns, stat tiles, and a critical-safety
banner → file it to the vault as an asset via vault-keeper, when that plugin is installed.
This command is only the manual trigger.
