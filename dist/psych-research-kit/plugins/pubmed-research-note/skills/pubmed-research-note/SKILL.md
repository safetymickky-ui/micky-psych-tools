...
description: >-
  ...
  literature say about", "search PubMed for", "is X true", "should I use X for Y" — or
  whether a drug, device, or intervention is
  ...
  (unpublished), Open Library (textbook gap), Wikipedia (terms). ALWAYS locks the decision
  FIRST before searching — intent-lock plugin if installed, else a short built-in interview;
  skip only on explicit opt-out ("just search"). By default writes the report and shows it
  inline; vault filing only via the vault-keeper plugin when installed; atomic notes ONLY on
  "atomize" / "ทำโน้ต". NOT for: daily multi-domain literature sweeps; non-biomedical
  research; MCQ/CRQ/Essay generation; grading; one-line lookups.
---
...
**If the `intent-lock` plugin is not installed** (standalone install of this kit), the gate
still runs — you conduct it yourself: ask the user only the few questions needed to fix the
decision, the verdict's shape, the scope boundary, and the anti-goal (see the brief anatomy
below), skipping anything the request already answers unambiguously. Do not silently guess in
place of asking; the opt-out rule above applies unchanged.

Whether locked by intent-lock, by your own interview in its absence, or, on opt-out, built by
you alone, the output of this step is a **decision brief** — see the next section. Read
...
`intent-lock` is a **separate plugin** — this whole section applies only when it is installed;
without it, Step 0's built-in fallback interview governs instead. It decides *what is being
asked*; this skill decides *what the evidence says about it* — never re-implement the
interview here. **Chain to it first on every request, unconditionally**, as Step 0 above
requires. It is not a fallback
...
3. **File (default):** hand the finished report to the vault-keeper skill to save as an
   **artifact** — it owns paths, dedup, MOC wiring, and the index. Pass: title
   ...
   to save (e.g. "don't vault this" / "no vault") — **or if the vault-keeper plugin is not
   installed** (standalone install of this kit): then steps 1–2 are the whole pipeline; say in
   the Close that no vault filing was attempted. Never improvise a vault of your own.
...
- A research request went to search without the decision being locked first — through
  intent-lock when installed, or the built-in brief interview otherwise — absent an explicit
  user opt-out; or the decision was guessed from ambiguous wording, or an opt-out was assumed
  from a request the user never actually opted out of.
...
- The finished report was not rendered inline, or — with vault-keeper installed — was not
  handed to it to file, absent an explicit opt-out like "don't vault this".
