...
is still a topic. That is not yours to resolve by guessing — it is exactly what Step 0's gate
(intent-lock when installed, your own brief interview otherwise) is there to settle. Go back
to Step 0.
...
by guessing — hand it back to Step 0's gate (intent-lock when installed, otherwise ask the
user directly), which exists precisely to settle it.
