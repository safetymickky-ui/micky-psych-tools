...
`intent-lock` is a separate plugin. **This whole file applies only when it is installed.**
Standalone (this kit ships without it), skip this contract entirely — SKILL.md Step 0's
built-in fallback interview governs instead. This file is the contract between the two, not a
