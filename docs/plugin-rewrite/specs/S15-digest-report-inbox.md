...
- **OBS empty-vault handoff drift** — empty-vault (S07) no longer calls digest-report directly; it transfers into the inbox (W2: a one-time copy, S07-W2-8; `drain_plan.py`, I12, only from W5 if the vault is kept — OQ15-a), and digest-report's survey picks up what lands there. No direct handshake code needed (§2.6).
...
| I12 | S07 | `drain_plan.py` writes directly into the two I09 paths and only reads `.intake-log.jsonl` (I10) to check "already landed" — never writes a line itself (S07 §2.7). Confirmed consistent. OQ15-a defers it to W5 (S07-W5-2, only if the vault is kept); the W2 copy (S07-W2-8) writes plain `research-notes/<slug>.md` files. |
