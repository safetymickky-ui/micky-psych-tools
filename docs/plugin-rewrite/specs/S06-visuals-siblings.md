...
- **Done ahead of wave 9f276bb (2026-09-26, quality pass):** every Change item landed in both files; remaining: the `auditInfographicResponsive` Done-when check (needs `$LEARN_HUB_DIR`). The same pass then scoped both files' breakpoints to `@media screen and (…)` (print stacked the columns — CI-05), so the `.mech` rule sits inside `@media screen and (max-width:720px|820px)`. Keep at W2/W3: `skills/clinical-infographic/scripts/verify-infographic.mjs` (Step 2.6 content check — S06-W2-1's `check-html.mjs --kind infographic` should absorb or call it, not drop it), literal file paths in `output-fidelity-and-safety` graders (the harness does not glob a file target; §4.1's `*.html` paths are superseded), and the clinical-class safety band in `source-contract.md`. Record: `docs/rewrite/baseline.md` `## Owner records`.
