...
- **Partly done 6c9a348 (2026-09-26, quality pass):** K13 (`grep -c Klaeng` = 0); remaining: the H45 slot split and the intent-lock-pairing.md mirror. The same pass added `references/evidence-checks.md` (provenance, appraisal, counter-search, label and safety, GRADE confidence, claim check) — add it to §2.4's reference table and budget; SKILL.md body is now ~5,230 tok (was ~4,740), so S03-W3-3's ≤4,000 cut is larger. Keep at W3: the literal file path in `output-sources-contract` graders (the harness does not glob a file target; §4.1's `*.md` paths are superseded); the CT.gov grader's tool prefix becomes `mcp__plugin_evidence_clinical-trials__`. Record: `docs/rewrite/baseline.md` `## Owner records`.
