...
| Eval canary | S11-W0-9's canary JSON (scaffold + `Bash(cat *)` grant) | `used-bash` and `read-seed` pass (done 2026-09-24, `w0d4.json`; delivery-log row d) |
...
| Baseline | `grep -c '^## W0 — ' docs/rewrite/baseline.md` (both repos; the dated section, not `## W0 smoke`) | 1 |
...
| sync-vault picks preflight | `bash scripts/eval-project-skill.sh sync-vault --smoke -- --allow-tools "Bash(npm run sync:preflight *),Bash(npm run sync:apply *),Bash(git status *),Bash(git log *)"` (S13 §4.4, Q33-a; case `sync-preflight-before-apply` is smoke-tagged) | pass |
...
| Q24 | Answered 2026-09-24 (S11-W0-9, delivery-log row d): `apt-get install -y bubblewrap socat` succeeds in the cloud container; the scaffold runs; a pattern grant (`Bash(cat *)`) runs; a whole-tool `--allow-tools Bash` run fails (`apply-seccomp: write /proc/self/uid_map: Operation not permitted`). See Q33. Original: does the install succeed, and does a Bash-granted eval run then pass? | — | W0 |
...
| Q30 | Half answered 2026-09-24: `claude plugin eval --json` reports `costUsd` per run and per file (probe USD 0.050 for one one-case run; the four check-d runs USD 0.46 in total). Still open: `claude -p`. Original: do `claude plugin eval --json` and `claude -p --output-format stream-json` report the cost of a run? S12-W0-9 sets the caps from the probe's cost per run, and a live pass stops at its cap (OQ3). | read `$E/w0d.json` and the run summary at S11-W0-9; read the last event of one `claude -p` stream at S12-W0-6 check i | W0 |
...
| Q33 | Answered 2026-09-24: **(a)**. Every eval grant names the commands a case runs, never the whole `Bash` tool: S01 release `"Bash(python3 *)"`; S07 smoke and release `"Write,AskUserQuestion,Bash(ls *),Bash(mkdir *),Bash(git status *),Bash(git log *),Bash(git diff *)"`; S13 smoke `"Bash(npm run sync:preflight *),Bash(npm run sync:apply *),Bash(git status *),Bash(git log *)"`; the other specs' grants were already patterns. Files are written through `Write`, because a pattern grant does not cover an output redirect. Trade-off: in the W0-d runs a pattern-granted `cat` ran where whole-tool Bash could not start, so pattern-granted commands run outside the OS sandbox; each grant is therefore kept to the commands the case needs, with no delete command, and the suites are owner-written (the same condition `--scaffold` already relies on). The first S12-W0-8 run confirms each grant: a `permission_denied` event in the trace names a missing one. Original: Smoke cases that grant the whole `Bash` tool cannot pass in the cloud container: the eval sandbox (bubblewrap + seccomp) cannot start there (Q24). Affected: S07's smoke grant `"Write,Bash,AskUserQuestion"` (vault-keeper, empty-vault), S13's `--allow-tools Bash` (sync-vault), and any other §4.4 command with a bare `Bash`. | — | W0 |
| Q34 | micky `.gitignore` (S10-W0-7) ignores `plugins/*/evals/results/`, but `scripts/eval.sh` (S12-W0-3) writes to `evals/results/` at the repo root. Closed 2026-09-24 by a follow-up commit that adds `evals/results/` to `.gitignore`. | — | W0 |
| Q35 | learn-hub's ratchet seeds a `windows-path` entry for `verify`'s `/home/user` path (SKILL.md line 42), found by skill-lint check 5 at S21-W0-1, and no spec step removes it, so it would block S12-W5-3's empty-ratchet rule. **Recommended:** S18 adds a step that replaces the path with `$LEARN_HUB_DIR` and closes the entry (`ratchet close`). ingest-article's `C:\` entries close at S17-W1-3. | the executor, before S18's W4 block | W4 |
| Q36 | The trigger locks record path-like tokens as `slash` phrases: `/vault` in 10 learn-hub skills and `/articles` in ingest-article and ingest-slides (micky's rule, mirrored). A description rewrite that drops one fails `triggers verify` unless the step runs `triggers remove`, and §5's lock table does not list them. **Recommended:** before W1's first description edit, restrict `kind: slash` extraction in both gates to tokens that name an existing command or skill, re-extract both locks (a shrink, logged in each repo's CHANGELOG). | the executor, W1 entry | W1 |
| Q37 | learn-hub's pre-commit hook gates commits that touch `.claude/skills/` or `CLAUDE.md` (per S21-W0-3), but not `plugins/*/skills/*/SKILL.md` (also a lock scan root) or a hand edit of `docs/rewrite/ratchet.json`. **Recommended:** widen the path filter to those two in S21's next hook touch; `npm run check:skills` in the wave exit gates covers the gap until then. | the executor | W1 |
