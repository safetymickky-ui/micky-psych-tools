...
| Q33 | Smoke cases that grant the whole `Bash` tool cannot pass in the cloud container: the eval sandbox (bubblewrap + seccomp) cannot start there (Q24). Affected: S07's smoke grant `"Write,Bash,AskUserQuestion"` (vault-keeper, empty-vault), S13's `--allow-tools Bash` (sync-vault), and any other §4.4 command with a bare `Bash`. Options: **(a) Recommended:** narrow each grant to the commands the case runs (`Bash(python3 *)`, `Bash(node *)`, no output redirects: a pattern grant does not cover `>`), and write files through a `Write` grant; **(b)** run those units' smoke under WSL2; **(c)** record them `MANUAL` in `## W0 smoke`. | the owner's answer before S12-W0-8 | W0, before S12-W0-8 |
