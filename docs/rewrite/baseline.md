...
- **2026-09-26 — plugin-creator 0.3.2.** Same branch, restarted from master after PR #44 merged. The owner asked in chat to fix CP-28's remaining false validator claims and CP-27's keyword graders. Released with `bump.py --write` per CLAUDE.md, overriding plan rule 8 as for 0.3.1; S08-W3-9's `release.py plugin-creator minor --write` still lands on `0.4.0`. The explicit-invoke graders changed again, so plugin-creator's S12-W0-8 smoke baseline still must run `--against pre-rewrite`.
